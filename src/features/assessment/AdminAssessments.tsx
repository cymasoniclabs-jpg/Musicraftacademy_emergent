...
import { exportToCSV } from './logic/scoring';
...
const AdminAssessments: React.FC = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (passcode === adminCode) {
      ...
      setError(t('adminAssessments.invalidPasscode'));
      setPasscode('');
    }
  };
  ...
  const exportAttempts = () => {
    if (attempts.length === 0) {
      alert(t('adminAssessments.noAttemptsToExport'));
      return;
    }
        ...
        >
          <div className="bg-gradient-to-br from-royal-900/50 to-silver-900/50 border border-royal-500/20 rounded-3xl backdrop-blur-md p-8">
            <div className="text-center mb-8">
              ...
              <h1 className="font-inter text-2xl text-white mb-2">
                {t('adminAssessments.loginTitle')}
              </h1>
              <p className="font-source text-gray-300">
                {t('adminAssessments.loginSubtitle')}
              </p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-6">
              <div>
                <label className="block font-source font-medium text-white mb-2">
                  {t('adminAssessments.passcodeLabel')}
                </label>
                <div className="relative">
                  <input
                    ...
                    placeholder={t('adminAssessments.passcodePlaceholder')}
                    required
                  />
                  <button
                    type="button"
              ...
              <button
                type="submit"
                className="w-full bg-gradient-to-r from-royal-600 to-royal-400 text-white font-inter font-semibold text-lg py-3 rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-royal-500/25"
              >
                {t('adminAssessments.accessButton')}
              </button>
            </form>
          </div>
        </motion.div>
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex items-center justify-between mb-12">
        <div>
          <h1 className="font-poppins text-4xl md:text-5xl mb-4 bg-gradient-to-r from-royal-400 to-silver-300 bg-clip-text text-transparent font-bold">
            {t('adminAssessments.headerTitle')}
          </h1>
          <p className="font-source text-xl text-gray-300">
            {t('adminAssessments.headerSubtitle')}
          </p>
        </div>
        ...
        >
          {t('adminAssessments.logoutButton')}
        </button>
      </div>
      ...
      <div className="mb-8">
        <div className="flex space-x-1 bg-gray-800/50 rounded-2xl p-1">
          {[
            { id: 'overview', label: t('adminAssessments.overviewTab'), icon: BarChart3 },
            { id: 'config', label: t('adminAssessments.configTab'), icon: Settings },
            { id: 'exports', label: t('adminAssessments.exportsTab'), icon: Download }
          ].map((tab) => {
      ...
      <div className="space-y-8">
          ...
          >
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-gradient-to-br from-royal-900/30 to-royal-800/20 border border-royal-500/20 rounded-2xl backdrop-blur-md p-6 text-center">
                ...
                <div className="text-gray-300 font-inter">{t('adminAssessments.totalAttempts')}</div>
              </div>
              ...
              <div className="bg-gradient-to-br from-green-900/30 to-green-800/20 border border-green-500/20 rounded-2xl backdrop-blur-md p-6 text-center">
                ...
                <div className="text-gray-300 font-inter">{t('adminAssessments.completionRate')}</div>
              </div>
              ...
              <div className="bg-gradient-to-br from-blue-900/30 to-blue-800/20 border border-blue-500/20 rounded-2xl backdrop-blur-md p-6 text-center">
                ...
                <div className="text-gray-300 font-inter">{t('adminAssessments.averageScore')}</div>
              </div>
              ...
              <div className="bg-gradient-to-br from-purple-900/30 to-purple-800/20 border border-purple-500/20 rounded-2xl backdrop-blur-md p-6 text-center">
                ...
                <div className="text-gray-300 font-inter">{t('adminAssessments.completedAssessments')}</div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-royal-900/30 to-royal-800/20 border border-royal-500/20 rounded-2xl backdrop-blur-md p-8">
              <h2 className="font-inter text-2xl font-bold text-white mb-6">{t('adminAssessments.bandDistributionTitle')}</h2>
              <div className="grid grid-cols-3 gap-6">
                <div className="text-center">
                  <div className="w-16 h-16 bg-gradient-to-r from-green-500 to-emerald-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <span className="font-inter text-xl font-bold text-white">A</span>
                  </div>
                  <div className="text-2xl font-bold text-white mb-1">{bandDistribution.A}</div>
                  <div className="text-gray-300 font-inter">{t('adminAssessments.bandA')}</div>
                </div>
                
                <div className="text-center">
                  <div className="w-16 h-16 bg-gradient-to-r from-yellow-500 to-orange-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <span className="font-inter text-xl font-bold text-white">B</span>
                  </div>
                  <div className="text-2xl font-bold text-white mb-1">{bandDistribution.B}</div>
                  <div className="text-gray-300 font-inter">{t('adminAssessments.bandB')}</div>
                </div>
                ...
                <div className="text-center">
                  <div className="w-16 h-16 bg-gradient-to-r from-red-500 to-pink-500 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <span className="font-inter text-xl font-bold text-white">C</span>
                  </div>
                  <div className="text-2xl font-bold text-white mb-1">{bandDistribution.C}</div>
                  <div className="text-gray-300 font-inter">{t('adminAssessments.bandC')}</div>
                </div>
              </div>
            </div>
          </motion.div>
        ...
        {activeTab === 'config' && (
          ...
          >
            <div className="bg-gradient-to-br from-royal-900/30 to-royal-800/20 border border-royal-500/20 rounded-2xl backdrop-blur-md p-8">
              <h2 className="font-inter text-2xl font-bold text-white mb-6">{t('adminAssessments.configTitle')}</h2>
                  ...
                  <div key={section.id} className="border border-gray-700 rounded-xl p-6">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-royal-400 font-semibold">
                        {t('adminAssessments.configWeight', { weight: Math.round(section.weight * 100) })}
                      </span>
                    </div>
                    ...
                    <div className="text-sm text-gray-400">
                      {t('adminAssessments.configItems', {
              ...
              <div className="mt-8 p-4 bg-yellow-900/20 border border-yellow-500/30 rounded-xl">
                <p className="text-yellow-200 text-sm">
                  <strong>Note:</strong> {t('adminAssessments.configNote')}
                </p>
              </div>
            </div>
          </motion.div>
        ...
        {activeTab === 'exports' && (
          ...
          >
            <div className="bg-gradient-to-br from-royal-900/30 to-royal-800/20 border border-royal-500/20 rounded-2xl backdrop-blur-md p-8">
              <h2 className="font-poppins text-2xl font-bold text-white mb-6">{t('adminAssessments.exportTitle')}</h2>
              ...
              <div className="grid md:grid-cols-3 gap-6">
                <div className="text-center p-6 border border-gray-700 rounded-xl">
                  ...
                  <h3 className="font-inter text-xl font-semibold text-white mb-2">
                    {t('adminAssessments.exportAttemptsTitle')}
                  </h3>
                  <p className="text-gray-300 text-sm mb-4">
                    {t('adminAssessments.exportAttemptsDesc')}
                  </p>
                  <button
                  ...
                  >
                    ...
                    {t('adminAssessments.exportButton')}
                  </button>
                </div>
                ...
                <div className="text-center p-6 border border-gray-700 rounded-xl">
                  ...
                  <h3 className="font-inter text-xl font-semibold text-white mb-2">
                    {t('adminAssessments.exportScoresTitle')}
                  </h3>
                  <p className="text-gray-300 text-sm mb-4">
                    {t('adminAssessments.exportScoresDesc')}
                  </p>
                  <button
                ...
                <div className="text-center p-6 border border-gray-700 rounded-xl">
                  ...
                  <h3 className="font-inter text-xl font-semibold text-white mb-2">
                    {t('adminAssessments.exportAnswersTitle')}
                  </h3>
                  <p className="text-gray-300 text-sm mb-4">
                    {t('adminAssessments.exportAnswersDesc')}
                  </p>
                  <button
              ...
              {attempts.length === 0 && (
                <div className="mt-8 p-4 bg-gray-800/50 border border-gray-600 rounded-xl text-center">
                  <p className="text-gray-400">
                    {t('adminAssessments.noDataForExport')}
                  </p>
                </div>
              )}
            </div>
