...
import { calculateSectionScore, calculateOverallScore, getRecommendedLevel, getBand } from './logic/scoring';
import { submitEnquiryDual } from '../forms/utils/submitEnquiryDual';
...
const AssessmentWizard: React.FC = () => {
  ...
  const handleNext = () => {
    ...
    saveAnswer({
      ...
      sectionId: currentItemData.sectionId as any,
      value: finalAnswer,
      timestamp: Date.now(),
  ...
  const renderQuestion = () => {
    ...
    switch (type) {
        ...
        return (
          <div className="space-y-6">
            <div className="text-center">
              ...
              {description && (
                <p className="font-source text-gray-300 mb-6">{description}</p>
              )}
            </div>
            ...
            <div className="flex justify-center mb-8">
              <button
                ...
                className="group relative px-8 py-4 bg-gradient-to-r from-royal-600 to-royal-400 text-white font-inter font-semibold text-lg rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-royal-500/25 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <span className="relative flex items-center">
                  {isPlaying ? (
                    <>
                      <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin mr-2"></div>
                      {t('assessmentWizard.playingAudio')}
                    </>
                  ) : (
                    <>
                      <Play className="mr-2 h-5 w-5" />
                      {replaysLeft > 0 ? t('assessmentWizard.playAudio', { replaysLeft }) : t('assessmentWizard.noReplays')}
                    </>
                  )}
                </span>
              </button>
                ...
                ['Higher', 'Lower', 'Same'].map((option) => (
                  <label key={option} className="cursor-pointer">
                    ...
                    <div className={`p-4 rounded-xl border-2 text-center transition-all duration-300 ${
                      ...
                      {t(`assessmentWizard.pitchComparisonOptions.${option.toLowerCase()}`)}
                    </div>
                  </label>
                ))
              ) : (
                ['Same', 'Different'].map((option) => (
                  <label key={option} className="cursor-pointer">
                    ...
                    <div className={`p-4 rounded-xl border-2 text-center transition-all duration-300 ${
                      ...
                      {t(`assessmentWizard.audioComparisonOptions.${option.toLowerCase()}`)}
                    </div>
                  </label>
                ))
              )}
            ...
            {!timerActive ? (
              <div className="flex justify-center">
                <button
                  ...
                  className="group relative px-8 py-4 bg-gradient-to-r from-royal-600 to-royal-400 text-white font-inter font-semibold text-lg rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-royal-500/25 disabled:opacity-50"
                >
                  <span className="relative flex items-center">
                    <Play className="mr-2 h-5 w-5" />
                    {t('assessmentWizard.startTask')}
                  </span>
                </button>
              </div>
            ) : (
              <div className="text-center space-y-6">
                <div className="flex items-center justify-center space-x-4">
                  ...
                  <span className="font-inter text-2xl font-bold text-white">
                    {t('assessmentWizard.timedFocusClock', {
                ...
                <button
                  onClick={handleTap}
                  className="px-12 py-6 bg-gradient-to-r from-silver-600 to-silver-400 text-white font-inter font-bold text-xl rounded-xl hover:shadow-lg transition-all duration-300"
                >
                  {t('assessmentWizard.timedFocusTap')}
                </button>
                
                <p className="font-source text-gray-300">
                  {t('assessmentWizard.timedFocusCount', { count: focusCount })}
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center mb-8">
              ...
              >
                <span className="relative flex items-center">
                  <Play className="mr-2 h-5 w-5" />
                  {isPlaying ? t('assessmentWizard.playingAudio') : t('assessmentWizard.tapTempoPlayBeat', { replaysLeft })}
                </span>
              </button>
            </div>
            ...
            <div className="text-center space-y-6">
              <button
                onClick={handleTap}
                className="px-16 py-8 bg-gradient-to-r from-silver-600 to-silver-400 text-white font-inter font-bold text-2xl rounded-xl hover:shadow-lg transition-all duration-300"
              >
                {t('assessmentWizard.tapTempoTap')}
              </button>
              
              <p className="font-source text-gray-300">
                {t('assessmentWizard.tapTempoTaps', { taps: tapTimes.length })}
              </p>
            </div>
          </div>
        );
          ...
          <div className="space-y-6">
            ...
            <div className="flex justify-center mb-8">
              ...
              >
                <span className="relative flex items-center">
                  <Play className="mr-2 h-5 w-5" />
                  {isPlaying ? t('assessmentWizard.playingAudio') : t('assessmentWizard.playAudio', { replaysLeft })}
                </span>
              </button>
            </div>
            ...
            <div className="max-w-md mx-auto">
              <input
                ...
                value={userAnswer}
                onChange={(e) => handleAnswerChange(e.target.value)}
                placeholder={t('assessmentWizard.digitSpanPlaceholder')}
      ...
      default:
        return (
          <div className="text-center">
            <p className="font-source text-gray-300">{t('assessmentWizard.questionTypeNotImplemented')}</p>
            <p className="font-source text-white">{t('assessmentWizard.loading')}</p>
          </div>
        );
    }
  };
    ...
    return (
      <div className="min-h-screen pt-20 bg-gradient-to-b from-gray-900 to-gray-800 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-white">{t('assessmentWizard.loading')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="text-center mb-12">
        <h1 className="font-poppins text-4xl md:text-5xl mb-4 bg-gradient-to-r from-royal-400 to-silver-300 bg-clip-text text-transparent font-bold">
          {t('assessmentWizard.headerTitle')}
        </h1>
        <p className="font-inter text-xl text-silver-200 mb-2 font-medium">
          {t('assessmentWizard.headerSubtitle')}
        </p>
        <p className="font-source text-gray-300">
          {t('assessmentWizard.sectionProgress', {
      ...
      <div className="max-w-4xl mx-auto mb-12">
        <div className="flex items-center justify-between mb-4">
          <span className="font-source text-sm text-gray-400">
            {t('assessmentWizard.questionProgress', {
              current: currentItem + 1,
              total: totalItems
            })}
          </span>
          <span className="font-source text-sm text-gray-400">
            {t('assessmentWizard.overallProgress', { progress: Math.round(progress) })}
          </span>
        </div>
        ...
        >
          {isAudioEnabled ? (
            <>
              ...
              {t('assessmentWizard.audioControl.enabled')}
            </>
          ) : (
            <>
              <VolumeX className="mr-2 h-4 w-4" />
              {t('assessmentWizard.audioControl.disabled')}
            </>
          )}
        </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center justify-between">
          <button
            ...
            className="group relative px-6 py-3 border-2 border-gray-600 text-gray-400 font-inter font-semibold rounded-xl overflow-hidden transition-all duration-300 hover:text-white hover:border-gray-500 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:text-gray-400 disabled:hover:border-gray-600"
          >
            <span className="relative flex items-center">
              <ChevronLeft className="mr-2 h-5 w-5" />
              {t('assessmentWizard.previousButton')}
            </span>
          </button>

          <div className="flex items-center space-x-4">
            <span className="font-source text-gray-400">
          ...
          <button
            ...
            className="group relative px-6 py-3 bg-gradient-to-r from-royal-600 to-royal-400 text-white font-inter font-semibold rounded-xl overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-royal-500/25 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:shadow-none"
          >
            <span className="relative flex items-center">
              {currentSection === totalSections - 1 && currentItem === totalItems - 1 ? (
                <>
                  <CheckCircle className="mr-2 h-5 w-5" />
                  {t('assessmentWizard.completeButton')}
                </>
              ) : (
                <>
                  {t('assessmentWizard.nextButton')}
