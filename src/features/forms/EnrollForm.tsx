...
import { EnrolSchema, EnrolFormData } from './schemas';
...
const EnrollForm: React.FC = () => {
  ...
  const onSubmit = async (data: EnrolFormData) => {
    // Check honeypot
    if (data.honeypot) {
    ...
    try {
      ...
      const result = await submitEnquiryDual(
        {
          ...
          email: data.email,
          phone: data.phone,
          program: data.programLevel,
          intent: data.intent,
          message: `Age: ${data.age}${data.guardianName ? `, Guardian: ${data.guardianName} (${data.guardianPhone})` : ''}${data.accessibility ? `, Accessibility: ${data.accessibility}` : ''}\n\nGoals: ${data.goals}`,
      ...
      if (result.warnings.length > 0) {
        console.warn('Enrollment submission warnings:', result.warnings);
      }
              ...
              <div className="flex flex-col sm:flex-row gap-6 justify-center">
                ...
                >
                  <span className="relative flex items-center justify-center">
                    ...
                    {t('enrollForm.startAssessmentCta')}
                    <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
                  </span>
                </button>
                
                <button
                  onClick={() => navigate('/musicraft')}
                ...
                >
                  ...
                  <span className="relative flex items-center justify-center">
                    ...
                    {t('enrollForm.exploreProgramsCta')}
                  </span>
                </button>
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          ...
          <div className="space-y-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                ...
                <input
                  ...
                  placeholder={t('enrollForm.studentNamePlaceholder')}
                />
                {errors.fullName && (
              ...
              <div>
                ...
                <input
                  ...
                  max="99"
                  className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200"
                  placeholder={t('enrollForm.agePlaceholder')}
                />
                {errors.age && (
                  <p className="mt-2 text-red-400 text-sm" role="alert" aria-live="polite">
                    {errors.age.message}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label className="block font-source font-medium text-white mb-2">
                  {t('enrollForm.emailLabel')}
                </label>
                <input
                  {...register('email')}
                  type="email"
                  className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200"
                  placeholder={t('enrollForm.emailPlaceholder')}
                />
                {errors.email && (
                  <p className="mt-2 text-red-400 text-sm" role="alert" aria-live="polite">
              ...
              <div>
                <label className="block font-source font-medium text-white mb-2">
                  {t('enrollForm.phoneLabel')}
                </label>
                <input
                  {...register('phone')}
                  type="tel"
                  className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200"
                  placeholder={t('enrollForm.phonePlaceholder')}
                />
                {errors.phone && (
                  <p className="mt-2 text-red-400 text-sm" role="alert" aria-live="polite">
            ...
            {watchAge && watchAge < 18 && (
              ...
              >
                <h3 className="font-inter text-xl font-semibold text-white flex items-center">
                  ...
                  {t('enrollForm.guardianInfoHeading')}
                </h3>
                
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    <label className="block font-source font-medium text-white mb-2">
                      {t('enrollForm.guardianNameLabel')}
                    </label>
                    <input
                      {...register('guardianName')}
                      type="text"
                      className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200"
                      placeholder={t('enrollForm.guardianNamePlaceholder')}
                    />
                    {errors.guardianName && (
                      <p className="mt-2 text-red-400 text-sm" role="alert" aria-live="polite">
                  ...
                  <div>
                    <label className="block font-source font-medium text-white mb-2">
                      {t('enrollForm.guardianPhoneLabel')}
                    </label>
                    <input
                      {...register('guardianPhone')}
                      type="tel"
                      className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200"
                      placeholder={t('enrollForm.guardianPhonePlaceholder')}
                    />
          ...
          <div className="space-y-6">
            <h2 className="font-inter text-2xl font-semibold text-white flex items-center">
              ...
              {t('enrollForm.programSelectionHeading')}
            </h2>
            
            <div className="grid md:grid-cols-2 gap-6">
              <div>
                <label className="block font-source font-medium text-white mb-2">
                  {t('enrollForm.programLevelLabel')}
                </label>
                <select
                ...
                >
                  <option value="">{t('enrollForm.programLevelPlaceholder')}</option>
              ...
              <div>
                <label className="block font-source font-medium text-white mb-2">
                  {t('enrollForm.intentLabel')}
                </label>
                <select
                  {...register('intent')}
                  className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200"
                >
                  <option value="">{t('enrollForm.intentPlaceholder')}</option>
                  <option value="Enrol">Enrol now</option>
          ...
          <div className="space-y-6">
            <h2 className="font-inter text-2xl font-semibold text-white flex items-center">
              ...
              {t('enrollForm.additionalInfoHeading')}
            </h2>
            
            <div>
              <label className="block font-source font-medium text-white mb-2">
                {t('enrollForm.accessibilityNeedsLabel')}
              </label>
              <textarea
                ...
                placeholder={t('enrollForm.accessibilityNeedsPlaceholder')}
              />
            </div>
            
            <div>
              <label className="block font-source font-medium text-white mb-2">
                {t('enrollForm.messageGoalsLabel')}
              </label>
              <textarea
                {...register('goals')}
                rows={4}
                className="w-full px-4 py-3 bg-gray-800/50 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-royal-500 focus:border-transparent transition-all duration-200 resize-none"
                placeholder={t('enrollForm.messageGoalsPlaceholder')}
              />
              {errors.goals && (
                <p className="mt-2 text-red-400 text-sm" role="alert" aria-live="polite">
              ...
              <span className="relative flex items-center justify-center">
                {isSubmitting ? (
                  <>
                    ...
                    {t('enrollForm.submittingButton')}
                  </>
                ) : (
                  <>
                    <CheckCircle className="mr-2 h-5 w-5" />
                    {t('enrollForm.submitButton')}
                    <ArrowRight className="ml-2 h-5 w-5 group-hover:translate-x-1 transition-transform" />
                  </>
                )}
              </span>
